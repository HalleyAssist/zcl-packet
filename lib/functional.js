...
class FuncPayload {
    constructor(clusterId, direction, cmd){
        ...
        if (!this.command)
            throw new Error(`Unrecognized command (${this.cluster}, ${direction}, ${this.cmd})`)
    ...
    parse (zclBuf) {
        var knownBufLen = this.meta.knownBufLen;
        if(knownBufLen) parsedBufLen = knownBufLen

        return FuncPayload.parseFrame(zclBuf, this.meta.params)
    }
    ...
    frame (args) { // args can be an array or a value-object if given
        ...
        if (params) {
            if(this.meta.params.constructor.name !== 'Map'){
                //is multirsp
...
FuncPayload.parseFrame = function (zclBuf, params){
    ...
    parser = (new DChunks()).join(chunkRules).compile({ once: true });
