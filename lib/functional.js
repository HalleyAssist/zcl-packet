...
FuncPayload.parseFrame = function(zclBuf, callback, params){
    var chunkRules = [],
        err,
        params,
        parser,
...
FuncPayload.prototype.parse = function (zclBuf, callback) {
    var knownBufLen = zclmeta.functional.get(this.cluster, this.cmd).knownBufLen;
    if(knownBufLen) parsedBufLen = knownBufLen

    var params = zclmeta.functional.getParams(this.cluster, this.cmd);

    return FuncPayload.parseFrame(zclBuf, callback, params)
};
    ...
    args.forEach(function (arg, idx) { // arg: { name, type, value }
        ...
        switch (type) {
            ...
            case 'stringPreLen':
                ...
                break;
            case "data32":
                dataBuf = dataBuf.string(32, 'utf8');
            break;
            case 'dynUint8':
            case 'dynUint16':
...
FuncPayload.prototype.frame = function (args) { // args can be an array or a value-object if given
    ...
    return FuncPayload.buildFrame(args)
};
